...
function openProject(slug){activeProject=projects.find(p=>p.slug===slug);if(!activeProject)return;data={...defaultData(),...(activeProject.data||{})};$('#projects').classList.add('hidden');$('#editor').classList.remove('hidden');$('#active-project-name').textContent=activeProject.name;$('#public-link').href=$('#public-link-top').href=publicUrl(slug);render();scrollTo({top:0})}
...
$('#create-project').onclick=async()=>{const name=$('#new-name').value.trim(),slug=$('#new-slug').value.trim().toLowerCase().replace(/[^a-z0-9-]+/g,'-').replace(/^-|-$/g,'');if(!name||!slug){$('#project-message').textContent='請填寫專案名稱與網址代號。';return}$('#project-message').textContent='建立中…';const initial=defaultData();const {error}=await db.from('progress_projects').insert({name,slug,data:initial});if(error){$('#project-message').textContent=`建立失敗：${error.message}`;return}$('#new-name').value=$('#new-slug').value='';$('#project-message').textContent='專案已建立。';await loadProjects();openProject(slug)};
